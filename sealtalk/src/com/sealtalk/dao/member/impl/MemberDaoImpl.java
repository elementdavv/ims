...
public class MemberDaoImpl extends BaseDao<TMember, Integer> implements
	...
	public boolean valideOldPwd(String account, String oldPwd) {
		try {
			Criteria ctr = getCriteria();
			ctr.add(Restrictions.and(Restrictions.eq("account", account),
					Restrictions.eq("password", oldPwd)));
	...
	public List<TMember> getLimitMemberIds(int limit) {
		String sql = (new StringBuilder(
				"select new TMember(t.id) from TMember t")).toString();
	...
	public int delMemberByUserId(String ids) {
		try {
			String hql = (new StringBuilder("delete from TMember where id in (")
